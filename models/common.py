...
#         g = math.gcd(in_channels, out_channels)
#         self.dwconv = DepthwiseConv2d(in_channels, s, g)
#         self.pointconv = PointwiseConv2d(in_channels, out_channels)
#
#     def forward(self, x):
#         ret = self.dwconv(x)
#         out = self.pointconv(ret)
#         return out
#
#
#
...
#
#
# def DepthwiseConv2d(in_channels, stride=1, g=1):
...
#     return DiagonalwiseRefactorization(in_channels, stride, g)
#
#
...
class DPN_Block(nn.Module):
    ...
    def __init__(self, in_chnls, add_chnl, cat_chnl, cardinality, d, stride):
        ...
        self.conv1 = Conv(in_chnls, self.chnl, 1, 1, 0)
        self.conv2 = Conv(self.chnl, self.chnl, 3, stride, 1, g=cardinality)
        # self.conv2 = Conv(self.chnl, self.chnl, 3, stride, 1, g=1)
...
class BottleneckDPN(nn.Module):
    def __init__(self, in_chnls, add_chnl, block=1, cat_chnl=16, cardinality=16, d=3,
                 stride=1):  # ch_in, ch_out, number, shortcut, groups, expansion
        super(BottleneckDPN, self).__init__()
        self.cdty = cardinality
        self.chnl = in_chnls
        self.conv = self.__make_layers(block, add_chnl, cat_chnl, d, stride)
        self.bn = nn.BatchNorm2d(self.chnl)  # applied to cat(cv2, cv3)
        self.act = nn.LeakyReLU(0.1, inplace=True)
        self.conv2 = Conv(self.chnl, in_chnls, 1, 1)
...
class DPN(nn.Module):
    # DPN -C3
    def __init__(self, c1, c2, block=1, cat_chnl=16, cardinality=16, d=3,
                 stride=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # act=FReLU(c2)
        self.m = self.__make_layers(block, self.add_chnl, cat_chnl, d, stride)
        self.cv4 = Conv(self.chnl, c_, 1, 1)
    ...
    def __make_layers(self, block, add_chnl, cat_chnl, d, stride):
        ...
        for i, s in enumerate(strides):
            layers.append(DPN_Block(self.chnl, add_chnl, cat_chnl, self.cdty, d, s))
            self.chnl = self.add_chnl + cat_chnl
        return nn.Sequential(*layers)

    def forward(self, x):
        return self.cv3(torch.cat((self.cv4(self.m(self.cv1(x))), self.cv2(x)), dim=1))


class CBAMDPN(nn.Module):
    # DPN -C3
    def __init__(self, c1, c2, block=1, cat_chnl=16, cardinality=16, d=3,
                 stride=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super(CBAMDPN, self).__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cdty = cardinality
        self.chnl = c_
        self.add_chnl = c_
    ...
    def forward(self, x):
        return self.spatial_attention(
            self.channel_attention(self.cv3(torch.cat((self.cv4(self.m(self.cv1(x))), self.cv2(x)), dim=1))))
